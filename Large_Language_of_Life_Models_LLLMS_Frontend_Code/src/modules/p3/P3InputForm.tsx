...
const P3InputForm: React.FC<P3InputFormProps> = ({ onRun, isRunning }) => {
  const { register, control, handleSubmit, watch, formState: { errors } } = useForm<P3Form>({
    ...
    defaultValues: {
      experimentName: 'Drug Screen Alpha',
      projectId: 'proj-1',
      tissueType: 'Brain',
      diseaseCondition: 'Neurodegeneration',
      screeningMode: 'broad',
      maxCompounds: 100,
      filterToxicity: true,
      pathwayFocus: 'mTOR, Autophagy',
  ...
  return (
    <form onSubmit={handleSubmit(onRun)} className="flex flex-col h-full">
      <div className="border-b bg-slate-50 p-6">
        <h3 className="text-lg font-bold text-slate-900 text-scientific-purple">Multispecies & Tissue Engine Discovery</h3>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input {...register('experimentName')} className="mt-1 block w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-scientific-purple focus:outline-none" placeholder="e.g. My Screening" />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                {errors.tissueType && <p className="mt-1 text-xs text-scientific-red font-bold">{errors.tissueType.message}</p>}
              </div>
              <div>
                ...
                {errors.diseaseCondition && <p className="mt-1 text-xs text-scientific-red font-bold">{errors.diseaseCondition.message}</p>}
              </div>
            </div>
          </div>
          <div>
        ...
        <section className="space-y-4">
          ...
          <div>
            <label className="text-sm font-medium text-slate-700">Max Compounds to Screen</label>
            <input type="number" {...register('maxCompounds', { valueAsNumber: true })} className="mt-1 block w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700">Pathway Focus (comma separated)</label>
            <input type="text" {...register('pathwayFocus')} className="mt-1 block w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="e.g. Dopaminergic Synapse" />
          </div>
