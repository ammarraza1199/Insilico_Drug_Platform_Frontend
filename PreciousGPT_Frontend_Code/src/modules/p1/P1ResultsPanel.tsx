...
const P1ResultsPanel: React.FC<P1ResultsPanelProps> = ({ experimentId }) => {
  ...
  const { data: results, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        predictedBiologicalAge: d.predicted_biological_age || d.predictedBiologicalAge,
        chronologicalAge: d.chronological_age || d.chronologicalAge,
        ageAccelerationScore: d.age_acceleration_score || d.ageAccelerationScore,
        ageAccelerationClass: d.age_acceleration_class || d.ageAccelerationClass,
        shapGenes: (d.shap_genes || d.shapGenes || []).map((g: any) => ({
          gene: g.gene,
          shapValue: g.shap_value || g.shapValue,
          expressionLevel: g.expression_level || g.expressionLevel,
          direction: g.direction
        })),
        diseaseClassification: d.disease_classification || d.diseaseClassification,
        therapeuticTargets: (d.therapeutic_targets || d.therapeuticTargets || []).map((t: any) => ({
          gene: t.gene,
          score: t.score,
          knownDrugTarget: t.known_drug_target || t.knownDrugTarget,
          actionabilityScore: t.actionability_score || t.actionabilityScore,
          expressionLevel: t.expression_level || t.expressionLevel || 0
        }))
      } as P1Results;
    },
  });
